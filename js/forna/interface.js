...
function RNA(sequence, structure, header , start, circularizeExternal, newError) {
  var self = this;
  //console.log(["New RNA with: ", sequence, structure, header].join('\n'));

  self.header = ko.observable(header);
  self.done = ko.observable(false);
  self.circularizeExternal = circularizeExternal;
  ...
  self.json = ko.onDemandObservable( function() {
      ajax(serverURL + '/struct_positions', 'POST', JSON.stringify( {header: self.header(), seq: self.sequence(), struct: self.structure()} ), 10000).success( function(data) {
            ...
            r.circularizeExternal = self.circularizeExternal;
...
function RNAManager( done, newError ) {
    ...
    self.add = function(sequence, structure, header, circularizeExternal) {
        var start = 1;
        ...
        self.newMolecules.push(new RNA(sequence, structure, header, start, 
                                       circularizeExternal, reportError));
    };
    ...
    self.parseFasta = function(lines, done, circularizeExternal) {
      var fCircularizeExternal = true;

      if (arguments.length < 3)
          fCircularizeExternal = true
      else
          fCircularizeExternal = circularizeExternal;
      ...
      function tmpRNA () {
        ...
        me.structure = '';
        me.header = 'rna';
        me.circularizeExternal = fCircularizeExternal;
      }
      var rna;
        ...
        lines.forEach( function(line) {
          ...
          if (line.substring(0, 1) === '>') {
            ...
            if (rna !== undefined) {
              ...
              self.add(rna.sequence, rna.structure, rna.header, fCircularizeExternal);
            }
            rna = new tmpRNA();
...
...(((((((..((((((.........))))))......).((((((.......))))))..))))))...'
  ...
  self.inputFile = ko.observable(null);
  self.cursorPosition = ko.observable('');
  self.circularizeExternal = ko.observable(true);
    ...
    if (self.inputFile() !== null) {
      ...
      r.onload = function(e) {
        ...
        //console.log(content);
	      ...
	      rnaManager.parseFasta(lines, endFunction, self.circularizeExternal());
      };
      r.readAsText(self.inputFile());
    } else {
      rnaManager.parseFasta(lines, endFunction, self.circularizeExternal());
